/**
 * Concession Credit Deduction API
 *
 * POST /api/concession-credits/deduct
 *
 * Deducts from an athlete's concession credit balance.
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!athleteId || !campId || !amountCents || amountCents <= 0) {
      return NextResponse.json(
        { error: 'athleteId, campId, and a positive amountCents are required' },
        { status: 400 }
      )
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (credit.balanceCents < amountCents) {
        throw new Error(
          `Insufficient balance: ${credit.balanceCents} cents available, ${amountCents} cents requested`
        )
      }
    ...
    if (status === 500) {
      console.error('[POST /api/concession-credits/deduct] Error:', error)
    }
