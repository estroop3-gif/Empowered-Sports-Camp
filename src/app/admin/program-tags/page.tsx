...
export default function AdminProgramTagsPage() {
  ...
  async function loadTags() {
    try {
      const res = await fetch('/api/program-tags?all=1', { credentials: 'include' })
  ...
  function startEdit(tag: ProgramTag) {
    setEditingId(tag.id)
    setEditForm({
      name: tag.name,
      slug: tag.slug,
  ...
  async function handleUpdate(id: string) {
    ...
    try {
      const res = await fetch(`/api/program-tags?id=${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  async function handleToggleActive(tag: ProgramTag) {
    try {
      const res = await fetch(`/api/program-tags?id=${tag.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
  ...
  async function handleDelete(tag: ProgramTag) {
  ...
  return (
    <AdminLayout userRole="hq_admin" userName="Admin">
      <PageHeader
        title="Program Tags"
        description="Manage program types that appear in camp filters and dropdowns"
      />
      ...
      {/* Add New Form */}
      {showAddForm && (
        <ContentCard title="Add New Program Tag" accent="neon">
      ...
      {/* Tags Table */}
      <ContentCard title={`All Tags (${tags.length})`} accent="purple">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10">
                ...
                <th className="text-center py-3 px-4 text-xs font-bold uppercase tracking-wider text-white/50">Camps</th>
                ...
                <tr key={tag.id} className="hover:bg-white/[0.02] transition-colors">
                  {editingId === tag.id ? (
                    <>
                      ...
                      <td className="py-3 px-4 text-center text-sm text-white/50">{tag.campCount}</td>
                    ...
                    <>
                      ...
                      <td className="py-3 px-4 text-center">
                        <span className="text-sm text-white/50">{tag.campCount}</span>
                      </td>
                      <td className="py-3 px-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                          ...
                          <button
                            ...
                            title={tag.campCount > 0 ? `Cannot delete: ${tag.campCount} camp(s) use this tag` : 'Delete'}
                          >
              ...
              {tags.length === 0 && (
                <tr>
                  <td colSpan={6} className="py-12 text-center text-white/40">
                    No program tags found. Add one above.
                  </td>
                </tr>
              )}
            </tbody>
