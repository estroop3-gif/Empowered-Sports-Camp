...
export default function CreateCampPage() {
  ...
  const userTenantIdRef = useRef(userTenantId)
  userTenantIdRef.current = userTenantId

  // Build draft payload from current form state
  const buildDraftPayload = () => {
    const fd = formDataRef.current
    const tenantId = fd.tenant_id || userTenantIdRef.current || ''
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (draftCampIdRef.current) {
        // Update the existing draft with final data (change status from draft if needed)
        response = await fetch(`/api/admin/camps?id=${draftCampIdRef.current}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formData),
        })
      } else {
        response = await fetch('/api/admin/camps?action=create', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(formData),
        })
      ...
      const data = await response.json()
      const campId = data.camp?.id || draftCampIdRef.current
      router.replace(`/portal/camps/${campId}/schedule`)
    } catch (err) {
