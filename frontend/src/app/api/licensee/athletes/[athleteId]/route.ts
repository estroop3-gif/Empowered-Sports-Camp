/**
 * Licensee Single Athlete API
 *
 * GET /api/licensee/athletes/[athleteId] - Get athlete details
 * PUT /api/licensee/athletes/[athleteId] - Update athlete
 * DELETE /api/licensee/athletes/[athleteId] - Archive athlete
 */
  ...
  try {
    ...
    if (error) {
      if (error.message.includes('Access denied')) {
        return NextResponse.json({ error: 'Athlete not in your territory' }, { status: 403 })
      }
    ...
    console.error('[API] GET /api/licensee/athletes/[id] error:', error)
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error('[API] PUT /api/licensee/athletes/[id] error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error('[API] DELETE /api/licensee/athletes/[id] error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
