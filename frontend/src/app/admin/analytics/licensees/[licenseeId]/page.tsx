...
export default function LicenseeDrilldownPage() {
  const params = useParams()
  const licenseeId = params.licenseeId as string
  const { user } = useAuth()
  ...
  const fetchTenantInfo = async () => {
    try {
      const res = await fetch(`/api/licensees/${licenseeId}`)
      if (res.ok) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error('Failed to fetch licensee analytics:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <AdminLayout userRole="hq_admin" userName={userName}>
      <PageHeader
        title={tenantInfo?.name || 'Licensee Analytics'}
        description="Detailed performance metrics for this licensee"
        breadcrumbs={[
          { label: 'Admin', href: '/admin' },
          { label: 'Analytics', href: '/admin/analytics' },
          { label: 'Licensees', href: '/admin/analytics/licensees' },
          { label: tenantInfo?.name || 'Licensee' },
        ]}
      >
        ...
        items={[
          {
            ...
            icon: DollarSign,
            variant: 'neon',
            subLabel: `${overview?.sessionsHeld ?? 0} sessions held`,
          },
          {
            label: 'Royalty Paid',
      ...
      <div className="grid gap-8 lg:grid-cols-2 mt-8">
        ...
        <ContentCard title="Program Breakdown" accent="magenta">
          <ProgramBreakdownChart
      ...
      {/* Sessions Table */}
      <div className="mt-8">
        <ContentCard title="Session Performance" accent="purple">
